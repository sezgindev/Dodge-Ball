...
public class ProceduralImageEditor : ImageEditor
{
    ...
    protected override void OnEnable()
    {
        ...
        if ((target as ProceduralImage).GetComponent<ProceduralImageModifier>() != null)
        {
            selectedId = attrList.IndexOf(((ModifierID[])(target as ProceduralImage)
                .GetComponent<ProceduralImageModifier>().GetType().GetCustomAttributes(typeof(ModifierID), false))[0]);
        }
    ...
    protected void ProceduralImageSpriteGUI()
    {
        ...
        {
            Sprite s = (Sprite)EditorGUILayout.ObjectField("Sprite",
                EmptySprite.IsEmptySprite((Sprite)m_Sprite.objectReferenceValue) ? null : m_Sprite.objectReferenceValue,
    ...
    protected void ProceduralImageTypeGUI()
    {
        ...
        else
        {
            m_Type.enumValueIndex =
                Convert.ToInt32(
                    EditorGUILayout.EnumPopup(spriteTypeContent, (ProceduralImageType)m_Type.enumValueIndex));
        }
            ...
            if (EditorGUILayout.BeginFadeGroup(showFilled.faded))
            {
                ...
                switch ((Image.FillMethod)m_FillMethod.enumValueIndex)
                {
                    case Image.FillMethod.Horizontal:
                        m_FillOrigin.intValue = (int)(Image.OriginHorizontal)EditorGUILayout.EnumPopup("Fill Origin",
                            (Image.OriginHorizontal)m_FillOrigin.intValue);
                        break;
                    case Image.FillMethod.Vertical:
                        m_FillOrigin.intValue = (int)(Image.OriginVertical)EditorGUILayout.EnumPopup("Fill Origin",
